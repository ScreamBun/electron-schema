...
// Tokenize
import { DomNodeUpdate, PlaceholderJIDL } from './tokenize';
import { ErrorMsg, DomNodeTokenize, PlaceholderTokenize } from './tokenize/interfaces';

// Interfaces
import {
  ColorProps, JIDLInputProps, JIDLInputState, StyleProps, ObjectCSS
} from './interfaces';
...
class JIDLInput extends Component<JIDLInputProps, JIDLInputState> {
  // eslint-disable-next-line react/static-property-placement
  static defaultProps: JIDLInputProps = defaultProps;
  ...
  tokenizePlaceholderJIDL = PlaceholderJIDL;

  constructor(props: JIDLInputProps) {
    super(props);
    ...
    this.tokenizePlaceholderJIDL = this.tokenizePlaceholderJIDL.bind(this);
    ...
    if (!locale) {
      console.warn("[Deprecation Warning] You did not provide a 'locale' prop for your JIDL input - This will be required in a future version. English has been set as a default.");
    }
  }
  ...
  updateInternalProps() {
    ...
    if (colors) {
      colorsMix = {
        ...
        // Extra
        name: safeGet(colors, 'name', colorsMix.name) as string,
        type: safeGet(colors, 'type', colorsMix.type) as string,
        options: safeGet(colors, 'options', colorsMix.options) as string
      };
    }
  ...
  showPlaceholder() {
    ...
    const unexpectedDataType = placeholderDataType !== 'string';
    if (unexpectedDataType) {
      err.throwError('showPlaceholder', 'placeholder', 'to be a string');
    }
  ...
  tokenize(obj: string|React.RefObject<HTMLSpanElement>): null|DomNodeTokenize|PlaceholderTokenize {
    const objDataType = getType(obj);
    if (!['string', 'object'].includes(objDataType)) {
      throw new TypeError(`tokenize() expects string or object type properties only. Got '${typeof obj}' type instead.`);
    }
    const { locale } = this.props;
    ...
    if (typeof obj === 'string') {
      return this.tokenizePlaceholderJIDL(obj);
    }
